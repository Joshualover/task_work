...
- **一个家庭可以有多个家长账号**（如爸爸 + 妈妈），权限完全相同：
  1. 家长端「孩子管理」→「家长账号」→「添加家长账号」（直接建号或重置密码）；
  2. 另一人在登录页选「我是家长」注册时填该**家庭邀请码**，即加入同一家庭（不填则新建家庭）。
- 两种开通孩子账号的方式：① 孩子在登录页注册并填邀请码；② **家长在「孩子管理」里直接为孩子创建账号/重置密码**（无需邀请码）。
- 任何账号都可以在右上角「修改密码」修改自己的密码。
