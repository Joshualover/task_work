...
- **完成反馈**：孩子点「我完成了」/ 目标达成时播放**撒花特效**并弹出鼓励语；提交与审批后**保持当前滚动位置**（不因列表刷新跳回顶部）；可**多选一提交**，家长可一键全部通过
- **登录与角色**：家长 / 孩子账号登录；**同一家庭支持多个家长账号**（爸爸、妈妈等权限相同）；家长可查看所有孩子的完成情况，孩子只能看自己且**无法进入家长模式**
- **布置人标识**：多个家长账号同时布置任务时，孩子端与家长端每条任务都会标注「**爸爸 / 妈妈 布置**」（必要任务沿用模板创建者，AI 导入的任务也会记录确认人），便于孩子知道该向谁交待
- **奖励限额**：每个奖励可设兑奖频率（每天 / 每周 / 每月）与每周期次数、积分上限
- **作业顺延**：布置作业时可勾选「遇周末/节假日顺延」并填天数，顺延期内不算逾期；截止日落在周五时一键顺延到周日
- **番茄钟（孩子端）**：专注 / 短休息 / 长休息，可关联今日任务；每 4 个番茄自动长休息，记录每日番茄数（跨页面常驻、后台计时）
- **任务记录日历**：家长端「任务记录」/ 孩子端「记录」Tab，按月日历查看每日任务完成情况（完成/进行中/逾期），点选某天查看当天任务明细
- **数据报表**：家长端「报表统计」用 **ECharts**（按需引入、懒加载）绘制近 4 周的**积分趋势**（渐变柱状图）与**完成率趋势**（平滑折线 + 面积），鼠标悬停有 tooltip；无数据时给出引导文案而不是白图
- **零花钱账单**：独立的零花钱账户（余额/收支流水）；奖励可设为「零花钱」类型（兑换并审批通过后计入收入）；孩子使用零花钱需发起申请，家长审批通过后才扣减
...
> 首次访问会进入**登录页**：先「注册」一个家长账号；孩子的账号用家长端「孩子管理」里的**家庭邀请码**注册。家里其他大人（如妈妈）也可以在「孩子管理 → 家长账号」里直接添加，或用邀请码自行注册加入（权限与家长相同）。详细说明见 [DEPLOY.md](./DEPLOY.md) 的「登录与角色」。
...
> 作业任务池：按 **单项任务 / 多项任务**（含子任务）分类筛选。多项任务可展开子任务并逐项勾选（家长端任务池、孩子端首页均可）；**子任务全部勾选完成后，该作业任务自动提交为「待确认」，需家长审批通过才算完成并发积分**。所有作业任务都必须家长审批，不存在自动完成。

> 目标型任务：新建任务时可选「目标任务」，设定**目标值与单位**（如「跳绳 100 个」「读 5 本书」），**可设截止时间，也可不限时间**；孩子分次记录进度（可负向撤销），**进度达到目标后自动提交为「待确认」，家长审批通过后得分**。未完成的目标任务不会因日期变化而消失，会一直显示至完成为止。

> 多选批量提交：孩子端任务列表右上角可切换「多选提交」，勾选多个已完成的任务（子任务未全部完成的不可选，目标型任务单独记录进度）后底部「一键提交」一起交给家长，家长只收到一条合并提醒。家长端「待确认」区在超过 1 条时可点「全部通过」批量审核并发积分（需要驳回某一个时单独处理）。

> 家长提醒：孩子**提交任务 / 达成目标 / 提交兑换申请 / 申请使用零花钱**时，家长端顶部铃铛会收到提醒（未读角标 + 列表，最多 30 秒自动刷新），点击可标记已读。

> 逾期补提交（**补提交期限 1 天**）：任务逾期后不会立刻从列表消失，而是进入补提交期。
>
> - **必要任务**：当天未完成 → 次日算逾期，逾期当天（任务日 +1 天）内可补提交
> - **作业 / 目标（设了截止日）**：截止日 + 顺延天数之后算逾期，逾期当天内可补提交
> - **作业 / 目标（未设截止日）**：视为不限时间，不会逾期（可一直完成）
>
> 补提交期内孩子可点「申请补提交」并填写说明，提交后标记 `is_late_submit`，家长端显示「补提交」角标与说明，**仍需家长审批通过才计入积分**；驳回则不再展示。**超过补提交期限后任务自动从任务池 / 孩子端消失，提交接口也会拒绝**（`补提交期限已过`）；历史记录里仍可按日期查看。逾期当天孩子端会显示「补提交截止 YYYY-MM-DD」。
