...
> 逆期补提交：任务逆期后不会从列表消失（**未完成的旧任务会跨天一直保留**，家长任务池与孩子端首页都能看到）。孩子可对逆期任务点「申请补提交」并填写说明，提交后标记 `is_late_submit`，家长端显示「补提交」角标与说明，**仍需家长审批通过才计入积分**；审批通过后该任务才移出列表（驳回则不再展示）。孩子端首页对逆期任务会给出红色提醒。
