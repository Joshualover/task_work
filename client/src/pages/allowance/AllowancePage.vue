  ...
  <div class="min-h-full bg-[#FFF7E6] p-4 sm:p-6">
    ...
    >
      <div class="flex items-center justify-between">
        <div>
          <p class="text-sm text-white/80">零花钱余额</p>
        ...
        >
          <PiggyBank class="h-8 w-8 text-white" />
        </div>
      </div>
    </div>
    ...
    >
      <h2 class="mb-4 text-lg font-semibold text-[#1F2329]">
        待审批申请
        <span class="ml-1 rounded-full bg-orange-100 px-2 py-0.5 text-xs text-[#FF8A3D]">
    ...
    <div v-if="tab === 'ledger'" class="rounded-2xl bg-white p-4 shadow-md">
      <h2 class="mb-4 text-lg font-semibold text-[#1F2329]">收支明细</h2>
      ...
      <div v-else class="space-y-2">
        ...
        >
          ...
          <div class="text-right">
            ...
            >
              {{ t.changeAmount > 0 ? '+' : '-' }}¥{{
                fenToYuan(Math.abs(t.changeAmount))
              }}
            </p>
            <p class="text-xs text-gray-500">
              余额 ¥{{ fenToYuan(t.balanceAfter) }}
            </p>
          </div>
        </div>
      </div>
    ...
    <div v-else class="rounded-2xl bg-white p-4 shadow-md">
      <h2 class="mb-4 text-lg font-semibold text-[#1F2329]">申请记录</h2>
    ...
    >
      <div class="space-y-4">
        ...
        >
          将{{ adjustDelta > 0 ? '增加' : '减少' }} ¥{{
            fenToYuan(Math.abs(adjustDelta))
          }}
        </p>
      </div>
      <template #footer>
